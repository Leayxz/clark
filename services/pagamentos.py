...
from infra.lnmarkets import LNMarket, LNMarketResult
...
class Pagamento:
      ...
      def __init__(self, cache, log_user, provider_pagamento: LNMarket) -> None:
            self._cache = cache
      ...
      async def gerar_invoice(self, username):
            ...
            deposito = await self._provider_pagamento.gerar_novo_deposito(preco_em_sats=preco_em_sats)
            if not deposito.sucesso or not deposito.payment_request: return PagamentoResult(status_code=500, mensagem=f"❌ Erro ao gerar novo depósito.")
            ...
            return PagamentoResult(status_code=200, qrcode=qrc_base64, id_invoice=deposito.deposit_id, payment_request=deposito.payment_request)
