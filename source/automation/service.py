...
class AutomationService:
    ...
    def get_automation_overview(self, exchange: str, email: str) -> tuple[ConfigurationDTO, ApiDTO, bool]:
        configuration = self._cache_repository.get_configuration(exchange, email)
        api = self._cache_repository.get_api(exchange, email)
        automation_status = self._cache_repository.get_status_automation(exchange, email)
    ...
    def enable_automation(self, exchange: str, email: str) -> None:
        ...
        self._cache_repository.add_activated_automation(exchange, email)
        payload = {"type": AutomationEvent.STARTED, "email": email, "exchange": exchange}
        self._cache_repository.publish_event(Channel.AUTOMATION, payload)
    ...
    def save_configuration(self, exchange, email: str, configuration: ConfigurationDTO):
        ...
        self._cache_repository.save_configuration(exchange, email, configuration)
